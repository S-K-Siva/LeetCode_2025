    ...
    public:
        ...
        vector<int> minimumCost(int n, vector<vector<int>>& edges, vector<vector<int>>& queries) {
            ...
            for (auto edge : edges) {
                ...
                int sourceRoot = findParent(source);
                int targetRoot = findParent(target);
            ...
            for (auto& q : queries) {
                ...
                else if (findParent(start) != findParent(end)) result.push_back(-1);
                else result.push_back(minPathCost[findParent(start)]);
            }
